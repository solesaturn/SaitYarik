...
export function kitSectionCopy(product: Pick<KitProduct, "kitRole">) {
  if (product.kitRole === "frame") {
    return {
      title: "Модули для этой рамки",
      text: "Только того же цвета. Число модулей равно числу постов рамки.",
    };
  }
  if (product.kitRole === "mechanism") {
    return {
      title: "Что взять в комплект",
      text: "Рамка того же цвета и другие модули на свободные посты. Отдельно от рамки модуль не используется.",
    };
  }
  return {
    title: "Что взять в комплект",
    text: "Готовое одинарное изделие устанавливается без отдельной рамки. Для блока на 2, 3 или 4 поста нужны рамка и модули того же цвета.",
  };
}
