      ...
      {kind && posts && color && (
        <section className="rounded-2xl bg-[#111] p-4 text-white sm:p-6 lg:p-8">
          {valid ? (
                ...
                {kitPriced ? (
                  <button type="button" className="btn btn-light w-full sm:w-auto" onClick={addKit}>
                    {added ? (
                      ...
                      "Добавить блок в корзину"
                    )}
                  </button>
                ) : (
                  <p className="text-sm text-white/60">Цена блока уточняется. Можно открыть карточки товаров.</p>
                )}
                {mechanism && (
            ...
            <p className="text-sm text-white/70">Такого блока нет: механизм и рамка должны совпасть по цвету и числу мест.</p>
          )}
        </section>
      )}
    </div>
