  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--line)] bg-[var(--paper)]/90 pt-[env(safe-area-inset-top)] backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center gap-2 px-4 py-2.5 sm:gap-4 sm:py-3.5">
        <button type="button" className="min-h-11 min-w-11 lg:hidden" onClick={() => setOpen(true)} aria-label="Меню">
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-black/30 lg:hidden" onClick={() => setOpen(false)}>
          <div
            className="flex h-full w-[min(100%,20rem)] flex-col bg-[var(--paper)] p-5 pt-[max(1.25rem,env(safe-area-inset-top))]"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="mb-6 flex items-center justify-between">
              <span className="text-lg font-semibold">{SITE.name}</span>
              <button type="button" className="min-h-11 min-w-11" onClick={() => setOpen(false)} aria-label="Закрыть">
                <X className="h-5 w-5" strokeWidth={1.5} />
              </button>
            </div>
            <div className="flex flex-col gap-1 overflow-y-auto text-base">
              {nav.map((item) => (
